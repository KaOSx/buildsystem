...
upload_and_recreate_db()
{
	...
	msg "adding packages to _temp"
        local pkg_names=$(ls _repo/local --ignore="local*.db*" | sed -nr 's/(-[^-]*){3}\.pkg\..*z$//p')
...
upload_packages()
{
	...
	pkgz_to_upload=$(ls _repo/local --ignore="local*.db*" | cut -d/ -f2)
	upload_list=$(ls _repo/local --ignore="local*.db*" | cut -d/ -f2 | grep ".xz$")
